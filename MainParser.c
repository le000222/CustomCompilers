...
rid_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern rid_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern rid_int line; /* source code line number - defined in scanner.c */
...
extern rid_void startParser(rid_void);
extern rid_int startScanner(ReaderPointer sc_buf);

static rid_void printParserError(rid_char* fmt, ...);
static rid_void displayParser(ReaderPointer ptrBuffer);
static rid_long getParserFileSize(rid_char* fname);
static rid_void callGarbageCollector(rid_void);
...
rid_int mainParser(rid_int argc, rid_char** argv) {
	...
	rid_int loadsize = 0; /*the size of the file loaded in the buffer */
...
rid_void printParserError(rid_char* fmt, ...) {
	...
	(rid_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
rid_void displayParser(ReaderPointer ptrBuffer) {
...
rid_void callGarbageCollector(rid_void) {
